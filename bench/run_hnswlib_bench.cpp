...
bool parse_argment(int argc, char **argv, Option &option) {
  int n;
...
void show_options(const Option &opt) {
...
#pragma omp parallel for
    ...
    if (!opt.gt_file_path.empty()) {
      NeighborsTableT gt;
      readGroundTruth(opt.gt_file_path, queries.size(), 10, gt);

      {
        auto scores = getExactRecallScores(results, gt);
      ...
      {
        auto scores = getExactRecallScoresWithDistanceTies(results, gt);
